...
The first `input.dirs` entry is local to the generated artifact; later entries and every `input.dirs.extra` entry are upstream.
...
For ordinary evidence, the current module's earlier input value wins a conflict, while the predecessor can fill an
...
A predecessor's `@Keep` protects the complete member and is written to the successor as `@Imported`.
That marker carries the protected contract through another immediate-predecessor hop without turning it into a local
successor decision.
After changing or removing the original `@Keep`, regenerate every successor in order: each `@Imported` contract is refreshed
or removed from its upstream source in both generation modes.
If one successor intentionally needs a different complete contract, replace its `@Imported` with a locally reviewed `@Keep`;
that local marker wins over all upstream inputs.
