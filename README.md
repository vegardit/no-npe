...
mvn compile -am -pl eea/commons-io-2
...
mvn compile -Deea-generator.action=generate -am -pl eea/commons-io-2
```
...
1. Create a new sub-directory under `eea/` using the name pattern `[LIBRARY_NAME]-[LIBRARY_MAJOR_VERSION]`
...
1. Add an eea-generator.properties in the same directory where you specify the relevant packages of the library.
   ...
   ```properties
   packages=org.example.cool-library.api,org.example.cool-library.spi
   ```
...
1. Run the generator inside parent project to create the EEA files
   ...
   ```bash
   mvn compile -Deea-generator.action=generate -am -pl eea/cool-library-2
   ```

1. Manually add the null/non-null annotations in the generated EEA files under `src/main/resources`
